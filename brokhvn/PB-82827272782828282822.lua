-- ══════════════════════════════════════
--   Tent Orbit v6.4 | by BmSky
--   + Wings 2 (Eagle Wing Bloom+Orbit)
--   + Fix Wings orientasi target player
-- ══════════════════════════════════════
...
header.Text = "  ▼  Tent Orbit v6.4  |  BmSky"
...
header.MouseButton1Click:Connect(function()
    ...
    header.Text = string.format("  %s  Tent Orbit v6.4  |  BmSky", isMinimized and "▶" or "▼")
...
local MODES = {
    ...
    { id="wings2",    label="🦅 Wings 2",     color=Color3.fromRGB(140,220,255) },
...
-- Helper: ambil CFrame orientasi dari target yang sedang aktif (local player atau target player)
...
local function updateWings(dt)
    local tCF = getTargetCFrame()
    local tp  = getTargetPosition()
    if not tCF or not tp then return end
    ...
    -- Ambil orientation dari target yang benar (fix: dulu selalu pakai HRP lokal)
    local rightVec = tCF.RightVector
    local upVec    = Vector3.new(0, 1, 0)
    local backVec  = -tCF.LookVector
...
-- ══════════════════════════════════════
--   WINGS 2 – Eagle Wing: Gather → Bloom (kiri & kanan barengan)
--             → Flap naik-turun 3 dtk → Nutup 2 dtk → Ulangi 5x → Orbit
--
--   Phase 0  GATHER   → ngumpul di belakang (~0.5 dtk)
--   Phase 1  BLOOM    → mekar kiri & kanan barengan (0.5 dtk)
--   Phase 2  FLAP     → naik-turun continuous selama 3 dtk (sayap terbuka)
--   Phase 3  CLOSE    → nutup selama 2 dtk
--             → setelah 5x siklus (BLOOM+FLAP+CLOSE) → Phase 4
--   Phase 4  ORBIT    → orbit 1x berlawanan arah (~2.2 dtk)
--   Phase 5  RETURN   → balik numpuk → loop ke Phase 0
-- ══════════════════════════════════════
local wings2Phase      = 0
local wings2PhaseTime  = 0
local wings2FlapCount  = 0      -- sudah berapa siklus (0..4)
local wings2SubPhase   = 0      -- tidak dipakai lagi, biarkan untuk reset
local wings2OrbitAngle = {}
local wings2Positions  = {}
local wings2FlapAngle  = 0

-- Durasi tiap phase (detik)
local W2_GATHER  = 0.50   -- ngumpul
local W2_BLOOM   = 0.50   -- mekar kiri & kanan barengan
local W2_FLAP    = 3.00   -- naik turun sayap (terbuka)
local W2_CLOSE   = 2.00   -- nutup
local W2_ORBIT   = 2.20   -- orbit
local W2_RETURN  = 0.60   -- balik numpuk
local W2_CYCLES  = 5      -- jumlah siklus bloom→flap→close

-- Helper: hitung posisi open sayap (kiri & kanan simetris, bloomT 0→1)
-- flapT: -1 = kibas bawah, 0 = netral, +1 = kibas atas
local function getWings2Positions(n, tCF, tp, bloomT, flapT)
    local rightVec      = tCF.RightVector
    local backVec       = -tCF.LookVector
    local half          = math.ceil(n / 2)
    local featherSpread = math.max(1.3, ORBIT_RADIUS * 0.5)

    local gatherPos = Vector3.new(
        tp.X + backVec.X * 1.2,
        tp.Y + HEIGHT_OFFSET + 0.4,
        tp.Z + backVec.Z * 1.2
    )

    -- smoothstep untuk bloom kiri & kanan barengan
    local eased = bloomT * bloomT * (3 - 2 * bloomT)

    local positions = {}
    for i = 1, n do
        local isRight    = (i <= half)
        local featherIdx = isRight and i or (i - half)
        local sideSign   = isRight and 1 or -1
        local t = (featherIdx - 1) / math.max(half - 1, 1)  -- 0=pangkal, 1=ujung

        local xOpen = sideSign * featherIdx * featherSpread
        local yOpen = t * t * ORBIT_RADIUS * 0.85 + t * ORBIT_RADIUS * 0.15

        -- Kibas: ujung bergerak lebih jauh, ada ripple wave antar bulu
        local flapAmp = ORBIT_RADIUS * 0.50 * t
        local flapY   = flapT * flapAmp
        local ripple  = math.sin(t * math.pi * 0.6) * math.abs(flapT) * ORBIT_RADIUS * 0.12

        local openPos = Vector3.new(
            tp.X + rightVec.X * xOpen + backVec.X * 0.8,
            tp.Y + HEIGHT_OFFSET + yOpen + flapY + ripple,
            tp.Z + rightVec.Z * xOpen + backVec.Z * 0.8
        )

        -- Kiri & kanan bloom barengan (eased sama untuk semua i)
        positions[i] = gatherPos:Lerp(openPos, eased)
    end
    return positions
end

local function updateWings2(dt)
    local tCF = getTargetCFrame()
    local tp  = getTargetPosition()
    if not tCF or not tp then return end
    ...
    wings2PhaseTime = wings2PhaseTime + dt
    wings2FlapAngle = wings2FlapAngle + dt

    local lerpSp = math.clamp(ORBIT_SPEED * 10 * dt, 0, 1)
    local half   = math.ceil(n / 2)
    ...
    -- ── Phase 0: GATHER – ngumpul di belakang ────────────────────
    if wings2Phase == 0 then
        for i = 1, n do targetPositions[i] = gatherPos end
        if wings2PhaseTime >= W2_GATHER then
            wings2Phase     = 1
            wings2PhaseTime = 0
            wings2FlapCount = 0
        end

    -- ── Phase 1: BLOOM – mekar kiri & kanan barengan ──────────────
    elseif wings2Phase == 1 then
        local t = math.min(wings2PhaseTime / W2_BLOOM, 1)
        targetPositions = getWings2Positions(n, tCF, tp, t, 0)
        if wings2PhaseTime >= W2_BLOOM then
            wings2Phase     = 2
            wings2PhaseTime = 0
            wings2FlapAngle = 0
        end

    -- ── Phase 2: FLAP – naik turun sayap selama 3 dtk ─────────────
    elseif wings2Phase == 2 then
        -- Sinusoidal naik-turun continuous, sayap tetap terbuka (bloomT=1)
        -- Frekuensi ~1 Hz = 1 naik-turun per detik
        local flapT = math.sin(wings2FlapAngle * math.pi * 2 * 1.0)
        targetPositions = getWings2Positions(n, tCF, tp, 1, flapT)
        if wings2PhaseTime >= W2_FLAP then
            wings2Phase     = 3
            wings2PhaseTime = 0
        end

    -- ── Phase 3: CLOSE – nutup selama 2 dtk ──────────────────────
    elseif wings2Phase == 3 then
        local t = math.min(wings2PhaseTime / W2_CLOSE, 1)
        -- Dari terbuka (bloomT=1) nutup ke numpuk (bloomT=0)
        -- flapT 0 biar nutup mulus tanpa kibas
        targetPositions = getWings2Positions(n, tCF, tp, 1 - t, 0)
        if wings2PhaseTime >= W2_CLOSE then
            wings2FlapCount = wings2FlapCount + 1
            if wings2FlapCount >= W2_CYCLES then
                -- Sudah 5x siklus → orbit
                wings2Phase     = 4
                wings2PhaseTime = 0
                -- Init sudut orbit: semua dari belakang, menyebar kanan-kiri
                wings2OrbitAngle = {}
                for i = 1, n do
                    local isRight    = (i <= half)
                    local featherIdx = isRight and i or (i - half)
                    local spread = (featherIdx - 1) * (math.pi / math.max(half, 1)) * 0.25
                    wings2OrbitAngle[i] = isRight
                        and (-math.pi/2 + spread)
                        or  ( math.pi/2 - spread)
                end
            else
                -- Ulangi bloom → flap → close
                wings2Phase     = 1
                wings2PhaseTime = 0
            end
        end

    -- ── Phase 4: ORBIT – orbit 1x berlawanan arah ─────────────────
    elseif wings2Phase == 4 then
        local orbitT = math.min(wings2PhaseTime / W2_ORBIT, 1)
        for i = 1, n do
            local isRight = (i <= half)
            local dir     = isRight and 1 or -1
            local baseA   = wings2OrbitAngle[i] or 0
            local angle   = baseA + dir * orbitT * math.pi * 2
            local yBob    = math.sin(orbitT * math.pi) * (ORBIT_RADIUS * 0.3)
            targetPositions[i] = Vector3.new(
                tp.X + math.cos(angle) * ORBIT_RADIUS,
                tp.Y + HEIGHT_OFFSET + yBob,
                tp.Z + math.sin(angle) * ORBIT_RADIUS
            )
        end
        if wings2PhaseTime >= W2_ORBIT then
            wings2Phase     = 5
            wings2PhaseTime = 0
        end

    -- ── Phase 5: RETURN – balik numpuk → loop ke Phase 0 ──────────
    elseif wings2Phase == 5 then
        local retT = math.min(wings2PhaseTime / W2_RETURN, 1)
        for i = 1, n do
            local isRight = (i <= half)
            local dir     = isRight and 1 or -1
            local baseA   = wings2OrbitAngle[i] or 0
            local angle   = baseA + dir * math.pi * 2
            local orbitEndPos = Vector3.new(
                tp.X + math.cos(angle) * ORBIT_RADIUS,
                tp.Y + HEIGHT_OFFSET,
                tp.Z + math.sin(angle) * ORBIT_RADIUS
            )
            local eased = retT * retT * (3 - 2 * retT)
            targetPositions[i] = orbitEndPos:Lerp(gatherPos, eased)
        end
        if wings2PhaseTime >= W2_RETURN then
            wings2Phase     = 0
            wings2PhaseTime = 0
        end
    end

    -- Apply lerp ke semua tenda
    for i = #tentFolders, 1, -1 do
        local f = tentFolders[i]
        if not f or not f.Parent then
            table.remove(tentFolders, i)
            table.remove(wings2Positions, i)
        else
            local tgt = targetPositions[i]
            if tgt then
                if not wings2Positions[i] then wings2Positions[i] = tgt end
                wings2Positions[i] = wings2Positions[i]:Lerp(tgt, lerpSp)
                local cp = wings2Positions[i]
                setCF(f, CFrame.new(cp.X, cp.Y, cp.Z))
            end
        end
    end
...
local modeTentCount = {
    orbit=2, leftright=2,
    love=8, tornado=8,
    ekor=6, games=6, wings=8, wings2=8,
...
bpIdRow.Position         = UDim2.new(0, 4, 0, 32)
...
bpIdBox.PlaceholderText  = "ID papan"
...
bpIdBox.FocusLost:Connect(function()
    bahlilSignID = bpIdBox.Text
end)

-- ── Baris Teks + Tombol Set ───────────
local bpTextRow = Instance.new("Frame", bahlilFrame)
bpTextRow.Size             = UDim2.new(1, -8, 0, 26)
bpTextRow.Position         = UDim2.new(0, 4, 0, 62)
bpTextRow.BackgroundColor3 = Color3.fromRGB(20, 15, 30)
bpTextRow.BorderSizePixel  = 0
bpTextRow.ZIndex           = 31
Instance.new("UICorner", bpTextRow).CornerRadius = UDim.new(0,5)

local bpTextBox = Instance.new("TextBox", bpTextRow)
bpTextBox.Size             = UDim2.new(1, -68, 1, -4)
bpTextBox.Position         = UDim2.new(0, 2, 0, 2)
bpTextBox.BackgroundColor3 = Color3.fromRGB(30, 24, 42)
bpTextBox.BorderSizePixel  = 0
bpTextBox.Text             = ""
bpTextBox.PlaceholderText  = "Ketik teks papan..."
bpTextBox.TextColor3       = Color3.fromRGB(255, 230, 180)
bpTextBox.PlaceholderColor3 = Color3.fromRGB(80, 70, 60)
bpTextBox.Font             = Enum.Font.Gotham
bpTextBox.TextSize         = 11
bpTextBox.ClearTextOnFocus = false
bpTextBox.ZIndex           = 33
Instance.new("UICorner", bpTextBox).CornerRadius = UDim.new(0,3)

local bpBtnSet = Instance.new("TextButton", bpTextRow)
bpBtnSet.Size             = UDim2.new(0, 60, 1, -4)
bpBtnSet.Position         = UDim2.new(1, -62, 0, 2)
bpBtnSet.BackgroundColor3 = Color3.fromRGB(50, 160, 80)
bpBtnSet.Text             = "✔ Set"
bpBtnSet.TextColor3       = Color3.new(1,1,1)
bpBtnSet.Font             = Enum.Font.GothamBold
bpBtnSet.TextSize         = 11
bpBtnSet.AutoButtonColor  = false
bpBtnSet.ZIndex           = 33
Instance.new("UICorner", bpBtnSet).CornerRadius = UDim.new(0,5)
...
-- Set: langsung kirim teks ke papan sekali
bpBtnSet.MouseButton1Click:Connect(function()
    local txt = bpTextBox.Text
    if txt == "" then return end
    bahlilSignID = bpIdBox.Text
    pcall(function()
        ReplicatedStorage.RE["1Cemeter1y"]:FireServer("ReturningBigSign3Name", bahlilSignID, txt)
    end)
    lblStatus.Text = "Set papan: " .. txt
    bpBtnSet.BackgroundColor3 = Color3.fromRGB(255, 210, 0)
    task.delay(0.4, function() bpBtnSet.BackgroundColor3 = Color3.fromRGB(50,160,80) end)
end)

-- Start/Stop: cycle teks default ke papan
...
local function setMode(newMode)
    ...
    if newMode == "wings2" then
        wings2Positions = {}; wings2Phase = 0; wings2PhaseTime = 0
        wings2OrbitAngle = {}; wings2FlapAngle = 0
        wings2FlapCount = 0; wings2SubPhase = 0
    end
...
LocalPlayer.CharacterAdded:Connect(function(newChar)
    ...
    gamesPositions = {}; gamesSides = {}; gamesSwapTimer = 0
    wingsPositions = {}; wingsAngle = 0
    wings2Positions = {}; wings2Phase = 0; wings2PhaseTime = 0; wings2FlapAngle = 0
    wings2FlapCount = 0; wings2SubPhase = 0
    mahkotaAngle = 0
    bodySideSides={}; bodySidePositions={}; bodySideOrbit={}; bodySideSwapTimer=0
    task.wait(1)
...
print("[OK] Tent Orbit v6.4 loaded –", PLAYER_NAME)
